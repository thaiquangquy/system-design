...
- **`check` package** — the hot path (`GET /api/v1/rate-limit/check?key=...&ip=...`), no auth. `key` identifies the route/resource and is matched against rules via `RuleCache`; `ip` is a separate, explicit dimension scoping the counter to one caller, so a single rule can cover a whole route while each IP tracks its own independent quota. `RateLimitService` composes the Redis counter keys from both (`key + ":ip:" + ip`), then calls `sliding_window.lua` atomically against the current + previous window counters to decide allow/deny and computes `X-RateLimit-*` response headers.
