...
class ShortenServiceTest {
    ...
    void returnsExistingShortCodeWhenLongUrlAlreadyKnown() {
        ...
        when(shortUrlOperations.findByLongUrl("https://example.com/already-shortened")).thenReturn(Optional.of(existing));
    ...
    void createsAndEncodesShortCodeForNewLongUrl() {
        ...
        when(shortUrlOperations.save(any(ShortUrl.class))).thenAnswer(invocation -> invocation.getArgument(0));
        ShortenService service = new ShortenService(shortUrlOperations, idTicketService);
