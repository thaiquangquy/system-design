 ...
 * connection for its whole duration, so switching {@link ShardRoutingContext} between calls
 * inside one transaction would silently keep using the first-resolved connection instead of
 * re-routing. This is also the honest model of a real sharded system — there's no cross-shard
 * ACID transaction here without a distributed transaction coordinator, which is out of scope.
 *
 * <p>{@code save}/{@code findBy...} are already individually transactional via Spring Data's
 * {@code SimpleJpaRepository}. Derived delete queries are not — Spring Data requires the caller
 * to supply the transaction for those — so {@code deleteExpired} wraps each shard's call in its
 * own {@link TransactionTemplate}-managed transaction.
 */
@Component
public class ShardedShortUrlOperations {
    ...
    public Optional<ShortUrl> findByShortUrl(String shortUrlCode) {
        return withRoute(routeFor(shortUrlCode, "replica"), () -> repository.findByShortUrl(shortUrlCode));
    }

    /**
     * longUrl isn't the shard key, so a lookup by longUrl can't be routed to a single shard —
     * scatter-gather across every shard's replica instead. Acceptable at the small shard counts
     * this "simulated locally" setup targets; a real deployment at scale would need a secondary
     * global index keyed by longUrl instead of doing this per request.
     */
    public Optional<ShortUrl> findByLongUrl(String longUrl) {
        ...
        for (String shardId : router.shardIds()) {
            Optional<ShortUrl> found = withRoute(shardId + "-replica", () -> repository.findByLongUrl(longUrl));
            if (found.isPresent()) {
                return found;
            }
        }
    ...
    private long deleteExpiredInTransaction(Instant cutoff) {
        Long deleted = transactionTemplate.execute(status -> repository.deleteByExpiresAtBefore(cutoff));
