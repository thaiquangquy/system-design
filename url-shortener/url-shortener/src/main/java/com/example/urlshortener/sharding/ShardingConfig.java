...
public class ShardingConfig {
    ...
    public ConsistentHashShardRouter consistentHashShardRouter(ShardingProperties properties) {
        return new ConsistentHashShardRouter(properties.shards().stream().map(ShardingProperties.Shard::id).toList());
    }
    ...
    @Bean
    @ConditionalOnProperty(prefix = "urlshortener.sharding", name = "enabled", havingValue = "true")
    public DataSource dataSource(ShardingProperties properties, DataSourceProperties defaultDataSourceProperties) {
        ShardRoutingDataSource routingDataSource = new ShardRoutingDataSource();
        Map<Object, Object> targets = new HashMap<>();
        for (ShardingProperties.Shard shard : properties.shards()) {
            targets.put(shard.id() + "-primary", build(shard.primaryUrl(), shard.primaryUsername(), shard.primaryPassword()));
            targets.put(shard.id() + "-replica", build(shard.replicaUrl(), shard.replicaUsername(), shard.replicaPassword()));
        }
