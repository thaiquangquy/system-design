...
/**
 * Per-IP throttling on POST /api/v1/shorten (design.md §8). Fixed-window counter in Redis: each
 * IP gets a key scoped to the current window (now / windowSeconds), incremented per request and
...
public class RateLimitFilter extends OncePerRequestFilter {
    ...
    public RateLimitFilter(
            ...
            @Value("${urlshortener.ratelimit.limit}") int limit,
            @Value("${urlshortener.ratelimit.window-seconds}") long windowSeconds) {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        if ("POST".equalsIgnoreCase(request.getMethod()) && SHORTEN_PATH.equals(request.getRequestURI())) {
